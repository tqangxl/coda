...
class WikiPlugin(KnowledgeSource, Plugin):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # 2. 启动核心引擎
            # 注意: WikiEngine 目前是同步初始化，但未来可能变为异步
            # 我们在这里执行 session.on_session_start()
            self.engine.session.on_session_start(session_id=f"plugin-{int(time.time())}")
    ...
    async def shutdown(self) -> None:
        ...
        if self.initialized:
            self.engine.session.on_session_end(key_findings=["Plugin session closed"])
    ...
    async def query(self, query: str, top_k: int = 5) -> List[dict[str, object]]:
        ...
        try:
            ...
            search_results = self.engine.search.search(query, limit=top_k)
            
            for res in search_results:
                results.append({
                    "source": res.get("file_path", "unknown"),
                    "content": res.get("snippet", res.get("body", ""))[:300],
