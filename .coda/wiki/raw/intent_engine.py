...
class IntentEngine:
    ...
    def __init_completion_tracker(self) -> None:
        ...
        if not hasattr(self, "_completed_phases"):
            ...
            self._active_roadmap: list[IntentResult] = []
  ...
  ...
    ...
    async def solidify_transitions(
        ...
        if valid_transitions:
            ...
            # 持久化到 SurrealDB
            persisted = False
            if persist_to_db and self._db and hasattr(self._db, "_db") and self._db.is_connected:
                try:
                    await self._db._db.query(
    ...
    async def load_solidified_from_db(self) -> int:
        ...
        if not self._db or not hasattr(self._db, "_db") or not self._db.is_connected:
            return 0

        try:
            result = await self._db._db.query(
                "SELECT intent_type, transitions FROM intent_transitions"
            )
            rows = self._db._extract_result(result) if hasattr(self._db, "_extract_result") else []
            loaded = 0
            for row in rows:
