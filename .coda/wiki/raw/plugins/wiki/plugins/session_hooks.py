...
class WikiDoctor(WikiPlugin):
    ...
    def _check_advisor_availability(self) -> WikiDiagnosis:
        """检查军师 (Advisor) 策略可用性。"""
        # 此处通常检查是否配置了 Opus 节点的 API Key 或 11001 端口的代理状态
        from .skill_tracker import GovernanceEngine
        advisor_hint = "Opus-3.5-Preview" # 假设的未来高阶模型
        return WikiDiagnosis(
            "advisor", True, f"Advisor strategy available: {advisor_hint}",
            severity="info"
        )
    ...
    def _fix(self, component: str) -> str | None:
        ...
        if component == "storage_structure":
            result = self._storage.initialize()
            return f"Initialized: {result}"
        ...
        if component == "drift":
            ...
            for path in orphaned:
                ...
                ).fetchone() if self._atlas._conn else None
                if node_data:
