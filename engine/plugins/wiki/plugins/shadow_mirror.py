...
class ShadowMirror(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        ...
        if self._storage:
            if not self._manifest:
                if hasattr(self._storage, "manifest"):
                    self._manifest = self._storage.manifest
    ...
    def create_shadow(self, filepath: Path) -> Path | None:
        """为 create_shadow 提供别名以维持向后兼容。"""
        return self.mirror_file(filepath)
    ...
    def mirror_file(self, filepath: Path) -> Path | None:
        ...
        # 写入 _meta/extracted/
        if not self._storage:
            logger.error("Storage not initialized for shadow mirror")
            return None
...
class RippleEngine(WikiPlugin):
    ...
    def compute_affected_set(self, changed_source: str | Path) -> list[str]:
        ...
        """
        if not self._manifest or not self._atlas:
            return []
        # Step 1: 直接受影响的页面
