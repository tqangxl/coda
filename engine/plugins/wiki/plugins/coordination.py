...
class HandoffManager(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._dir:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ HandoffManager: coordination_dir not available in context")
                return
            self._dir = coord_dir / "handoffs"
...
class AgentRegistry(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._dir:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ AgentRegistry: coordination_dir not available in context")
                return
            self._dir = coord_dir / "agents"
        self._dir.mkdir(parents=True, exist_ok=True)
...
class TaskBarrier(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._dir:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ BarrierManager: coordination_dir not available in context")
                return
            self._dir = coord_dir / "barriers"
        self._dir.mkdir(parents=True, exist_ok=True)
        logger.info("🚦 Barrier Manager plugin initialized")
...
class MemoryIsolation(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._root:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ MemoryIsolation: coordination_dir not available in context")
                return
            self._root = coord_dir / "memory"
...
class RetryQueue(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._dir:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ RetryManager: coordination_dir not available in context")
                return
            self._dir = coord_dir / "pending_retries"
        self._dir.mkdir(parents=True, exist_ok=True)
        logger.info("🔄 Retry Manager plugin initialized")
...
class AuditTrail(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._path:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ AuditLogger: coordination_dir not available in context")
                return
            self._path = coord_dir / "audit.md"
