...
class AtlasIndex(WikiPlugin):
    ...
    def __init__(self, db_path: str | Path | None = None, embedding_dim: int = 2048):
        ...
        # ── 联邦挂载注册表 ──
        # key: alias ("dept_wiki", "corp_wiki"), value: {"path": str, "layer": int, "project_id": str}
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        ...
        # ── 自动挂载联邦库 (来自 WIKI_MOUNTS 环境变量) ──
        self._auto_mount_from_env()

        logger.info(f"🗂️ Atlas plugin initialized: vec_enabled={self._vec_enabled}, mounts={len(self._mounts)}")
    ...
    def set_embed_fn(self, embed_fn: Callable[[list[str]], list[list[float]]]) -> None:
        ...
        logger.info("🎨 AtlasIndex: Embedding function attached.")

    @property
    def vec_enabled(self) -> bool:
        return self._vec_enabled

    def mount(self, alias: str, db_path: str) -> None:
        """挂载另一个 SQLite 知识库，实现联邦检索。"""
        if not self._conn:
            logger.error("Cannot mount: not connected")
            return
        ...
        try:
            ...
            if "corp" in path_lower or "org" in path_lower:
                layer = 1
            elif "dept" in path_lower or "team" in path_lower:
                layer = 2
            elif "public" in path_lower or "base" in path_lower:
                layer = 0
            ...
            logger.info(f"🔗 Mounted federated DB '{alias}' from {db_path} (Layer L{layer})")
        except Exception as e:
            logger.error(f"Failed to mount {db_path} as {alias}: {e}")

    def _auto_mount_from_env(self) -> None:
        """从 WIKI_MOUNTS 环境变量自动挂载知识库。"""
        mounts_str = os.getenv("WIKI_MOUNTS", "")
        if not mounts_str:
            return
            
        for idx, mount_def in enumerate(mounts_str.split(";")):
            mount_def = mount_def.strip()
            if not mount_def:
                continue
                
            parts = mount_def.split(":", 1)
            if len(parts) == 2:
                alias, path = parts[0].strip(), parts[1].strip()
            else:
                alias, path = f"mount_{idx}", mount_def
                
            self.mount(alias, path)

    def list_mounts(self) -> list[dict[str, Any]]:
        """获取所有已挂载的数据库。"""
        return [{"alias": k, **v} for k, v in self._mounts.items()]
        """是否启用了向量索引。"""
        return self._vec_enabled
    ...
    def connect(self) -> None:
        """打开数据库连接并初始化 Schema。"""
        if not self._db_path:
            logger.error("❌ AtlasIndex: _db_path is not set. Cannot connect.")
            return
        ...
        try:
            ...
            if "malformed" in str(e).lower():
                logger.warning(f"⚠️ Atlas database malformed: {self._db_path}. Recreating...")
                if self._conn:
                    try: self._conn.close()
                    except: pass
                
                # Backup and Delete
                backup_path = f"{self._db_path}.corrupted.{int(time.time())}"
                try:
                    ...
                    logger.error(f"❌ Critical: Failed to heal malformed database: {heal_e}")
                    raise e
            else:
                raise e
    ...
    def get_node(self, node_id: str) -> dict[str, Any] | None:
        ...
        # 尝试从 FTS 获取完整正文
        try:
            fts_row = self._conn.execute("SELECT body FROM node_fts WHERE node_id = ?", (node_id,)).fetchone()
            if fts_row:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "federated_mounts": len(self._mounts),
