...
class SkillTracker(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._path:
            ...
            if not coord_dir:
                logger.error("❌ SkillTracker: Coordination directory not found in context")
                return
            self._path = coord_dir / "skills.json"
        self._load()
...
class StepClaiming(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        if not self._lock_dir:
            coord_dir = ctx.coordination_dir
            if not coord_dir:
                logger.error("❌ StepClaiming: Coordination directory not found in context")
                return
            self._lock_dir = coord_dir / "locks"
...
class GovernanceEngine(WikiPlugin):
    ...
    def consult_advisor(
        ...
        model_hint: str | None = None,   # None = 完全由环境感知路由决定
    ) -> AdvisorAdvice:
        """
        ...
        model_hint: 传 None (推荐) 则由 ModelRegistry.resolve_model_hint()
                    自动选择当前环境可用的最优模型，彻底消除硬编码模型名依赖。
        """
        # 使用环境感知路由解析实际模型 ID (替代硬编码的 hint 字符串)
        resolved_model = model_hint
        if self._advisor_router and hasattr(self._advisor_router, "_registry"):
            try:
                resolved_model = self._advisor_router._registry.resolve_model_hint(
                    hint=model_hint,
                    specialty="reasoning",
                    scenario="advisor",
                )
            except Exception:
                resolved_model = model_hint or "(auto)"
        logger.info(f"🧠 Consulting Advisor (resolved={resolved_model}) for gate {gate.gate_id}")
