...
from ..akp_types import ManifestEntry, StorageLayer, KnowledgeNode, NodeType
...
class CompilationManifest:
    ...
    def get_missing_files(self) -> list[str]:
        """返回清单中存在但磁盘上已不存在的文件路径 (用于差异物理同步)。"""
        missing = []
        for path_str in self._entries.keys():
            # 检查文件物理存在性
...
class WikiStorage(WikiPlugin):
    ...
    async def initialize(self, ctx: WikiPluginContext) -> None:
        ...
        self.manifest = CompilationManifest(self.get_manifest_path())
    ...
    def _initialize_structure(self) -> dict[str, int]:
        ...
        dirs_to_create = [
            ...
            self._root / "_meta" / "coordination", # 协作目录
        ]
    ...
    def coordination_dir(self) -> Path:
        """Agent 协调目录。"""
        return self._root / "_meta" / "coordination"
    ...
    async def upsert_node(self, node: KnowledgeNode) -> Path:
        """[V7.0] 物理持久化知识节点到磁盘，并触发同步钩子。"""
        # 1. 确定目录 (根据 NodeType 映射到 CORE_SUBDIRS)
        type_to_dir = {
        ...
        subdir = type_to_dir.get(node.node_type, "entities")
        rel_path = f"{subdir}/{node.id}.md"

        # 2. 构造带 Frontmatter 的 Markdown 内容
        import yaml
        frontmatter = node.to_frontmatter_dict()
        # 移除 body 避免在 frontmatter 重复
        content = f"---\n{yaml.dump(frontmatter, allow_unicode=True, sort_keys=False)}---\n\n{node.body}"

        # 3. 写入物理文件
        filepath = self.write_knowledge_file(rel_path, content)

        # 4. 触发同步钩子 (让 AtlasIndex, SurrealAtlasIndex 等接管)
        if hasattr(self, "_ctx") and self._ctx and self._ctx.registry:
            await self._ctx.registry.dispatch(
                WikiHook.ON_NODE_INGEST, 
                {"node": node, "filepath": str(filepath)}
            )
