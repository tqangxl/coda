...
class IdentityRegistry:
    ...
    async def initialize(self) -> None:
        ...
        if self._store and self._store.is_connected:
            try:
                ...
                identity_data = await self._store.select_all("agent_identity")
                if identity_data:
    ...
    async def register(self, identity: SovereignIdentity) -> None:
        ...
        if self._store and self._store.is_connected:
            try:
                # 使用封装好的 upsert_identity 方法
                await self._store.upsert_identity(identity.did, identity.to_dict())
            except Exception as e:
    ...
    def verify_registry_health(self) -> dict[str, Any]:
        """
        [V7.0] 身份健康审计。
        检查是否存在 DID 冲突、孤立角色或低信任分身份。
        """
        issues = []
        dids = set()
        for ident in self._identities.values():
            if ident.did in dids:
                issues.append(f"Collision: Duplicate DID {ident.did}")
            dids.add(ident.did)
            
            if ident.trust_score < 0.5:
                issues.append(f"Low Trust: {ident.to_short_id()} score is {ident.trust_score}")
