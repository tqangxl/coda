...
class ToolExecutor:
    ...
    def __init__(self, working_dir: str = ".", mcp: "MCPManager | None" = None) -> None:
        self.working_dir = Path(working_dir).absolute()
        self._marker_seq = 0
        self._xml = XMLProtocol()
        self._sandbox = SecuritySandbox()
        self._mcp_manager = mcp
        self._mcp_registry = MCPRegistry() # Keep for config storage
        self._file_locks = {}
    ...
    def register_mcp_tool(self, server_name: str, tool_name: str, manager: "MCPManager") -> None:
        """注册一个 MCP 工具到调度器。"""
        # 允许原始名称和带前缀的名称调用
        prefixed_name = f"mcp_{server_name}_{tool_name}"
        
        async def _mcp_handler(args: dict[str, object]) -> str:
            res = await manager.call(server_name, tool_name, args)
            return res or f"ERROR: MCP tool {tool_name} returned empty or failed."
            
        self.dispatch[tool_name] = _mcp_handler
        self.dispatch[prefixed_name] = _mcp_handler
        logger.info(f"Registered MCP tool: {prefixed_name} (and {tool_name})")
