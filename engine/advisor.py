...
class ModelRegistry:
    ...
    def resolve_model_hint(
        ...
        """
        环境感知 + 配额压力分级路由 — V7.1 核心路由接口。

        在原有环境感知的基础上，增加配额压力感知：
        当 IDE 模型配额消耗到不同比例时，对不同调用场景执行不同的降级策略。

        配额压力阈値:
            NORMAL   (剩余 > 50%): 使用最优模型，无限制
            MODERATE (剩余 30-50%): 非关键场景降级到 WORKHORSE
            HIGH     (剩余 10-30%): 非关键场景降级到 SPEED；关键场景降 WORKHORSE
            CRITICAL (剩余 < 10%):  非关键场景跳过 LLM；关键场景降 LOCAL

        场景分类 (scenario 参数):
            "advisor"     — 军师咋询 (高优先级)
            "compiler"    — 编译分析 (中优先级)
            "enrichment"  — 富集增强 (低优先级，优先跳过)
            "search"      — 搜索摘要 (低优先级)
            "general"     — 通用 (中优先级)
        """
        # ── Step 1: 配额压力感知 ──
        pressure, remaining_pct = self._get_quota_pressure()
        if pressure != "NORMAL":
            logger.info(
                f"📊 Quota pressure={pressure} (remaining={remaining_pct:.1f}%) "
                f"scenario={scenario}"
            )

        # ── Step 2: 场景降级决策 ──
        adjusted_hint = self._apply_quota_pressure(hint, pressure, remaining_pct, scenario)
        if adjusted_hint == "__SKIP__":
            raise RuntimeError(
                f"[QuotaSkip] scenario='{scenario}' skipped: "
                f"pressure={pressure} remaining={remaining_pct:.1f}%"
            )

        # ── Step 3: IDE Bridge 实时模型列表 ──
        if os.getenv("Coda_AGENT") == "1":
            try:
                from .ide_bridge import IDEBridge
                bridge = IDEBridge._instance
                if bridge and bridge.is_connected:
                    ide_models = bridge.get_available_models()
                    if ide_models:
                        chosen_ide = self._pick_ide_model_by_pressure(
                            ide_models, pressure, scenario
                        )
                        logger.debug(
                            f"🎯 IDE Bridge: pressure={pressure} scenario={scenario} → {chosen_ide}"
                        )
                        return chosen_ide
            except (ImportError, Exception) as e:
                logger.debug(f"IDE Bridge unavailable ({e}), using registry fallback")

        # ── Step 4: 显式 hint 精确匹配 ──
        if adjusted_hint and adjusted_hint in self._models and self._models[adjusted_hint].is_available:
            logger.debug(f"🎯 Exact match → {adjusted_hint}")
            return adjusted_hint

        # ── Step 5: Tier 别名解析 ──
        tier_aliases: dict[str, ModelTier] = {
            "flagship":  ModelTier.FLAGSHIP,
            "opus":      ModelTier.FLAGSHIP,
            "high":      ModelTier.FLAGSHIP,
            "workhorse": ModelTier.WORKHORSE,
            "sonnet":    ModelTier.WORKHORSE,
            "medium":    ModelTier.WORKHORSE,
            "speed":     ModelTier.SPEED,
            "haiku":     ModelTier.SPEED,
            "fast":      ModelTier.SPEED,
            "local":     ModelTier.LOCAL,
        }
        target_tier = tier_aliases.get(
            adjusted_hint.lower() if adjusted_hint else "", None
        ) if adjusted_hint else None

        # ── Step 4: 从注册表找最优可用模型 ──
        best_candidates = self.find_best_advisor(
            specialty=specialty,
            budget_per_call_usd=budget_usd,
        )

        if target_tier:
            # 先在目标 tier 中找
            tier_candidates = [m for m in best_candidates if m.tier == target_tier]
            if tier_candidates:
                chosen = tier_candidates[0].model_id
                logger.debug(f"🎯 Tier={target_tier.value} → {chosen}")
                return chosen
            logger.debug(f"No {target_tier.value} models, fallback to best")

        # 直接取 Elo 排名最高的可用模型
        if best_candidates:
            chosen = best_candidates[0].model_id
            logger.debug(f"🎯 Best available → {chosen}")
            return chosen

        # ── 最终兜底: 任意可用模型 ──
        all_available = self.list_available()
        if all_available:
            chosen = all_available[0].model_id
            logger.warning(f"⚠️ Last-resort fallback → {chosen}")
            return chosen

        raise RuntimeError(
            "No available models. Configure an API key or run inside an IDE session."
        )
    ...
    def _get_quota_pressure(self) -> tuple[str, float]:
        """读取 IDEBridge 配额剩余比例，返回 (pressure_level, remaining_pct)。"""
        try:
            from .ide_bridge import IDEBridge
            bridge = IDEBridge._instance
            if bridge and bridge.is_connected:
                ...
                if snapshot and snapshot.models:
                    primary = min(
                        (m for m in snapshot.models if not m.is_exhausted),
                        key=lambda m: m.remaining_fraction,
                        default=None,
                    )
                    if primary:
                        pct = primary.remaining_percentage
                        if pct > 50:
                            return "NORMAL", pct
                        elif pct > 30:
                            return "MODERATE", pct
    ...
    def _apply_quota_pressure(
        ...
        """根据压力等级和场景返回调整后的 hint。返回 '__SKIP__' 表示该场景应跳过。"""
        if pressure == "NORMAL":
            return hint

        LOW  = {"enrichment", "search"}
        MID  = {"general", "compiler"}
        HIGH = {"advisor"}

        if pressure == "MODERATE":
            if scenario in LOW:
                logger.info(f"📉 MODERATE: {scenario} → speed")
                return "speed"
            if scenario in MID:
                logger.info(f"📉 MODERATE: {scenario} → workhorse")
                return "workhorse"

        elif pressure == "HIGH":
            if scenario in LOW:
                logger.warning(f"🚨 HIGH: skip {scenario}")
                return "__SKIP__"
            if scenario in MID:
                logger.warning(f"📉 HIGH: {scenario} → speed")
                return "speed"
            if scenario in HIGH:
                logger.warning(f"📉 HIGH: advisor → workhorse")
                return "workhorse"

        elif pressure == "CRITICAL":
            if scenario in LOW:
                logger.warning(f"🚨 CRITICAL: skip {scenario}")
                return "__SKIP__"
            if scenario in MID:
                logger.warning(f"🚨 CRITICAL: {scenario} → local")
                return "local"
            if scenario in HIGH:
                logger.warning(
                    f"🚨 CRITICAL ({remaining_pct:.1f}%): advisor → local"
                )
                return "local"

        return hint

    def _pick_ide_model_by_pressure(
        self,
        ide_models: list,
        pressure: str,
        scenario: str,
    ) -> str:
        """按压力等级和场景从 IDE 模型列表中选最合适的模型。"""
        LOW = {"enrichment", "search"}
        MID = {"general", "compiler"}

        if pressure == "NORMAL":
            return ide_models[0].model_id

        if pressure == "MODERATE" and scenario in LOW:
            return max(ide_models, key=lambda m: m.remaining_fraction).model_id

        if pressure in ("HIGH", "CRITICAL") and scenario in MID | LOW:
            lite = [
                m for m in ide_models
                if any(k in m.model_id.lower() for k in ("flash", "haiku", "lite"))
            ]
            pool = lite if lite else ide_models
            return max(pool, key=lambda m: m.remaining_fraction).model_id
...
class AdvisorExecutorRouter:
    ...
    async def _strategy_council(
        ...
        if not vote_tally:
            ...
            final_verdict = max(vote_tally, key=lambda k: vote_tally.get(k, 0))
