...
class AgentEngine:
    ...
    def __init__(
        ...
        **args: object,
        ...
        self.tools: ToolExecutor = ToolExecutor(str(self.working_dir), mcp=self.mcp_manager)
        ...
        # [V5.2] 从环境变量加载多路径 Skill 扫描
        skill_paths = [self.working_dir / "skills"]
        ...
        self._skill_factory: SkillFactory = SkillFactory(skill_paths)
        
        from .skill_forge import SkillForge
        self.skill_forge: SkillForge = SkillForge(
            llm=self._llm,
            skill_factory=self._skill_factory,
            db=self.db,
            target_skills_dir=skill_paths[0] if skill_paths else Path("skills"),
            project_id=self.store.state.wiki_config.get("project_id", "Coda_core") if self.store and getattr(self.store.state, "wiki_config", None) else "Coda_core"
        )
    ...
    async def _init_mflow_mcp(self) -> None:
        """初始化 M-flow MCP 服务器集成。"""
        m_flow_path = Path("D:/ai/docs/m_flow/m_flow-mcp")
        if not m_flow_path.exists():
            # 尝试相对路径或从环境变量获取
            m_flow_path = self.working_dir / "m_flow-mcp"
            
        if m_flow_path.exists():
            logger.info(f"🧬 [M-flow] 检测到 M-flow 目录，正在启动 MCP 服务...")
            # 构造启动命令: python -m src.server
            # 注意: 需要确保 PYTHONPATH 包含 src
            success = await self.mcp_manager.add_server(
                "m_flow",
                [sys.executable, "-u", "src/server.py", "--no-migration"], 
                env={"PYTHONPATH": str(m_flow_path), "M_FLOW_HOME": str(m_flow_path.parent)},
                cwd=str(m_flow_path)
            )
            
            if success:
                logger.info("✅ [M-flow] MCP 服务器启动成功。")
                # 动态注册工具到 ToolExecutor
                all_mcp_tools = self.mcp_manager.get_all_tools()
                for tool in all_mcp_tools:
                    if tool.get("server_name") == "m_flow":
                        self.tools.register_mcp_tool("m_flow", tool["original_name"], self.mcp_manager)
            else:
                logger.error("❌ [M-flow] MCP 服务器启动失败。")
        else:
            logger.warning(f"⚠️ [M-flow] 未找到 M-flow 路径 ({m_flow_path})，跳过集成。")
    ...
    async def initialize(self, db_url: str | None = None) -> None:
        ...
        # 7. [V7.0] 物理环境探测 (Hardening Check)
        if not (self.working_dir / ".git").exists():
            logger.warning("🚨 [物理审计缺失]: 当前工作目录未初始化 Git，物理回滚保护失效。")
        else:
            logger.info("🛡️ [物理审计激活]: Git Checkpoint 系统就绪。")
            
        # 8. [V7.0] M-flow 认知知识图谱集成 (MCP Server)
        await self._init_mflow_mcp()
            
        self._is_verified = True
        logger.info(f"✨ Coda Engine V7.0 fully initialized and verified for {self.agent_id}.")
    ...
    async def run(self, user_message: str, verdict: AdvisorVerdict | None = None) -> str:
        """运行自主循环 (V7.0 Hardened)."""
        # [V5.2 Hardening] 确保引擎已完全初始化 ， [ Traceability] 支持注入军师裁决 (AdvisorVerdict) 以实现全链路溯源。
        ...
        # ── V7.0: 物理恢复锚点 (Checkpointing) ──
        checkpoint_id = await self._create_physical_checkpoint()
        ...
        try:
            ...
            # 2. 核心执行循环
            final_answer = await self._run_loop(user_message)
            
            # 3. 三维度物理审计 (Zero-Mock QA)
            qa_res = await self._perform_triple_qa(user_message, final_answer)
            
            if not qa_res.success:
                logger.warning(f"🚨 Triple-QA REJECTED work: {qa_res.reason}\nIssues:\n{qa_res.report}")
                if self.store.state.beta_flags.get("auto_revert", True):
                    await self._rollback_to_checkpoint(checkpoint_id)
                    return f"❌ [QA 拒绝]: {qa_res.reason}\n理由: {qa_res.report}"
                else:
                    self._messages.append({"role": "user", "content": f"QA Rejected: {qa_res.reason}. Please fix."})
                    # 继续执行或退出取决于策略，这里默认结束并返回错误
                    return f"❌ [QA 拒绝]: {qa_res.reason}\n{qa_res.report}"
            
            # 审计通过，固化变更
            await self._commit_physical_changes(user_message)
            self.store.update("qa_passed", True)
            return final_answer
            
        except Exception as e:
            logger.error(f"Engine fatal error: {e}", exc_info=True)
            await self._rollback_to_checkpoint(checkpoint_id)
            raise
        finally:
    ...
    async def _run_loop(self, user_message: str) -> str:
        ...
        while step_count < self.store.state.max_iterations:
            ...
            if step_count % 3 == 0 or progress.is_stuck:
                ...
                reflect_verdict = str(reflection.get("verdict", "continue"))
                
                if reflect_verdict == "converged":
                    logger.info(f"🎯 HTAS: Goal converged ({reflection.get('honest_progress', 0)}%). Terminating.")
                    break
                elif reflect_verdict == "stuck_pivot":
            ...
            if step_count == 1 or (progress.is_stuck and self.last_intent and self.last_intent.execution_path == ExecutionPath.MAS):
                ...
                # [V7.2] SkillForge RECALL: 检索是否已有固化的成功经验
                if step_count == 1:
                    recall_result = await self.skill_forge.recall(user_message)
                    if recall_result:
                        self._messages.append({
                            "role": "system",
                            "content": f"🛠️ [SkillForge 记忆召回]: 我曾解决过类似任务 (置信度 {recall_result.confidence:.0%})。以下是成功经验与执行路径，请优先参考避免重复踩坑：\n\n{recall_result.skill_content}"
                        })
            ...
            if response.tool_calls:
                for tc in response.tool_calls:
                    res = await self._execute_tool_with_hooks(self.agent_id, tc) # type: ignore
    ...
    async def _perform_triple_qa(self, context: str, summary: str) -> QAResult:
        """
        [V7.0] 真正的三维度物理审计 (Zero-Mock Implementation).
        验证物理变更、合规性扫描与语义一致性。
        """
        logger.info("🔍 Initiating Triple-QA Physical Audit...")
        issues = []
        
        # --- 1. 物理增量审计 (Physical Delta) ---
        modified = self.htas.modified_files
        if not modified:
            # 如果意图是修改代码但没改，则视作可能存在的名义实现
            if any(k in context.lower() for k in ["replace", "fix", "update", "create", "modify"]):
                return QAResult(success=False, reason="No physical changes detected", report="The instruction implied code modification, but no files were physically changed on disk.")
        
        # --- 2. 指令合规审计 (Anti-Placeholder Scan) ---
        forbidden = ["TODO", "FIXME", "MOCK", "PLACEHOLDER", "STEP_PLACEHOLDER", "名义实现","实际","占位","Null","HACK","XXX","BUG","Not implemented","Coming soon","Placeholder","Mock data","模拟","Fake data","Dummy","Dummy data","Stub","Fallback","Workaround","Temporary","Quick fix"]
            ...
            if full_path.exists():
                try:
                    ...
                    for word in forbidden:
                        if word in content:
                            issues.append(f"Forbidden token '{word}' found in {file_path}")
                except Exception as e:
                    logger.warning(f"QA: Could not read {file_path}: {e}")

        # --- 3. 语义一致性审计 ---
        if len(summary) < 20 and len(modified) > 0:
            # 如果改了很多代码但总结非常简短，可能存在逃避式总结
            issues.append("Result summary suspiciously brief for the amount of code changed.")

        if issues:
            report = "\n".join([f"- {i}" for i in issues])
            return QAResult(success=False, reason="Compliance Audit Failed", report=report)

        return QAResult(success=True, reason="Verified", report="All physical and compliance checks passed.")

    async def _create_physical_checkpoint(self) -> str:
        """创建 Git 恢复点锚点。"""
        try:
            if not (self.working_dir / ".git").exists(): return "none"
            res = subprocess.run(["git", "rev-parse", "HEAD"], cwd=self.working_dir, capture_output=True, text=True)
            return res.stdout.strip()
        except Exception as e:
            logger.error(f"Failed to create checkpoint: {e}")
            return "none"

    async def _rollback_to_checkpoint(self, cid: str) -> None:
        """回滚所有物理变更。"""
        if cid == "none": return
        logger.warning(f"⏪ Rolling back to {cid[:8]} due to audit failure or crash...")
        try:
            subprocess.run(["git", "reset", "--hard", cid], cwd=self.working_dir, check=True)
            subprocess.run(["git", "clean", "-fd"], cwd=self.working_dir, check=True)
        except Exception as e:
            logger.error(f"Critical Rollback Error: {e}")

    async def _commit_physical_changes(self, msg: str) -> None:
        """固化物理变更。"""
        if not (self.working_dir / ".git").exists(): return
        try:
            subprocess.run(["git", "add", "."], cwd=self.working_dir, check=True)
            subprocess.run(["git", "commit", "-m", f"Audit Passed: {msg[:100]}"], cwd=self.working_dir)
            logger.info("💾 Physical changes committed and locked.")
        except Exception as e:
    ...
    async def _inject_semantic_recall(self, query: str) -> None:
        ...
        # 2b. [V7.0] 检索 M-flow MCP 知识图谱 (Episodic/Procedural)
        if "m_flow" in self.mcp_manager._connections:
            try:
                # 使用简化 query 接口，聚焦于 question 语义
                m_flow_res = await self.mcp_manager.call("m_flow", "query", {"question": query, "top_k": 3})
                if m_flow_res:
                    combined_content.append(f"[M-flow Memory]: {m_flow_res}")
            except Exception as e:
                logger.warning(f"M-flow recall failed: {e}")
        ...
        if self.htas.modified_files and len(self.htas.modified_files) > 0:
            ...
            try:
                ...
                if commit_hash:
                    ...
                    # [V7.2] SkillForge FORGE: 任务成功后，尝试将经验固化为新 Skill
                    # 传入用户初始任务目标，由于 _messages 第一个元素通常是 system，这里提取 user
                    initial_task = ""
                    for m in self._messages:
                        if m.get("role") == "user":
                            initial_task = str(m.get("content", ""))
                            break
                    
                    if initial_task:
                        await self.skill_forge.crystallize(
                            task=initial_task,
                            messages=self._messages,
                            modified_files=list(self.htas.modified_files)
                        )
