...
class FamilyOfficeLedger:
    ...
    async def initialize(self) -> None:
        ...
        if self._store and self._store.is_connected:
            try:
                # 聚合每个 Team 的总消耗
                ledger_data = await self._store.execute_query("SELECT team_id, math::sum(cost) as total_used FROM agent_ledger GROUP BY team_id")
                ...
                res_root = await self._store.execute_query("SELECT math::sum(cost) as grand_total FROM agent_ledger GROUP ALL")
                grand_total = res_root[0]['grand_total'] if res_root else 0
    ...
    async def check_and_record(self, identity: SovereignIdentity, cost: float) -> bool:
        ...
        if self._store and self._store.is_connected:
            try:
                ...
                await self._store.create_record("agent_ledger", record)
            except Exception as e:
